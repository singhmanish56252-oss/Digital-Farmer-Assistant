...
const CropCalendar = () => {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div>
        <p style={{ fontSize: 11, color: 'var(--primary)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>Step-by-Step Farming Guide</p>
        <h2 style={{ fontSize: '1.75rem', fontWeight: 900, color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>Crop Calendar & Reminders</h2>
        <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 4 }}>Track every farming task from sowing to harvest</p>
      </div>
      ...
      <div className="glass-card p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <div>
            <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 8 }}>Select Crop</label>
            <div className="flex gap-2">
              {CROPS.map(c => (
                <button key={c} onClick={() => setCrop(c)}
                  style={{
                    flex: 1, padding: '10px', borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: 'pointer', border: 'none',
                    background: crop === c ? 'var(--primary-light)' : 'rgba(255,255,255,0.04)',
                    color: crop === c ? 'var(--primary)' : 'var(--text-muted)',
                    outline: crop === c ? '1px solid var(--primary)' : '1px solid var(--border-light)',
                    transition: 'all 0.2s'
                  }}>
                  {c === 'Wheat' ? '🌾' : '🍚'} {c}
                </button>
          ...
          <div>
            <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 8 }}>Sowing Date</label>
            <input type="date" value={sowingDate} onChange={e => setSowingDate(e.target.value)} className="input-dark" />
          </div>
          <div>
            <div className="flex justify-between mb-3">
              <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Current Day</label>
              <span style={{ fontSize: 14, fontWeight: 800, color: 'var(--primary)' }}>Day {currentDay}</span>
            </div>
      ...
      <div className="glass-card p-6">
        <div className="flex justify-between mb-3">
          <div>
            <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>Season Progress</p>
            <p style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Day {currentDay} of {totalDays} · {Math.round(progressPct)}% complete</p>
          </div>
          <div className="text-right">
            <p style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Harvest in</p>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-3">
            <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 16 }}>Full Season Timeline</h3>
            {tasks.map((task, i) => {
                ...
                <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span>{tc.icon}</span>
                      <h4 style={{ fontSize: 13, fontWeight: 800, color: isDone ? 'var(--text-muted)' : 'var(--text-primary)' }}>{task.task}</h4>
                      ...
                      <span style={{ fontSize: 10, marginLeft: 'auto', color: 'var(--text-muted)', fontWeight: 600 }}>{getTaskDate(task.day)}</span>
                    </div>
                    <p style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.6 }}>{task.desc}</p>
                  </div>
                </motion.div>
              );
            })}
          ...
          <div className="space-y-4">
            <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Upcoming Tasks</h3>
              ...
              return (
                <div key={i} className="glass-card p-4" style={{ border: `1px solid ${tc.color}25` }}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Day {task.day}</span>
                  </div>
                  <h4 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 4 }}>{task.task}</h4>
                  <p style={{ fontSize: 11, color: 'var(--text-secondary)' }}>{getTaskDate(task.day)}</p>
                </div>
              );
            })}

            <div className="glass-card p-5" style={{ background: 'var(--primary-light)', border: '1px solid var(--primary)' }}>
              <h4 style={{ fontSize: 13, fontWeight: 700, color: 'var(--primary)', marginBottom: 8 }}>✓ Completed</h4>
              <p style={{ fontSize: '2rem', fontWeight: 900, color: 'var(--text-primary)' }}>{completedTasks.length}</p>
              <p style={{ fontSize: 11, color: 'var(--text-secondary)' }}>tasks done out of {tasks.length}</p>
            </div>
          </div>
        </div>
      )}
